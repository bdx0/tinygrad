...
from tinygrad.helpers import CI, DEBUG, fetch, getenv
...
def init_whisper(model_name="tiny.en", batch_size=1):
    if pathlib.Path(model_name).is_file():
        filename = model_name
    else:
...
if __name__ == "__main__":
    model, enc = init_whisper(
        "../whisper_bdx0/small.pt",
        # "tiny",
        # "small.en" if getenv("SMALL") else "tiny.en",
        batch_size=1,
    )
    ...
    if len(sys.argv) > 1:
        ...
        while True:
            ...
            if dec.endswith("<|endoftext|>"):
                ...
                eot_count = 0
                lst
            # if silence or no voice active, then clear audio queue
            # Detect silence
            # https://stackoverflow.com/questions/40896370/detecting-the-index-of-silence-from-a-given-audio-file-using-python
            # sound = AudioSegment(waveform, frame_rate=RATE, sample_width=4, channels=1)
            # dBFS = sound.dBFS
            ...
            # )
            # if len(silence) > 0 or eot_count > 3:
            if eot_count > 3:
                eot_count = 0
                total = None
